function combined_image = PTKCombineLeftAndRightImages(template, left, right, left_and_right_lungs)
    ...
    elseif ~isempty(right.RawImage)
        combined_image = right.Copy;
        combined_image.ResizeToMatch(template);
        combined_raw_image = combined_image.RawImage;
        right_mask = left_and_right_lungs.RawImage == 1;
        left_mask = left_and_right_lungs.RawImage == 2;
        combined_raw_image(~right_mask) = 0;
        combined_image.Clear;
        combined_image.ChangeSubImage(left);
        
        combined_raw_image(left_mask) = combined_image.RawImage(left_mask);
        combined_image.ChangeRawImage(combined_raw_image);
    else
        combined_image = template;
    end
end
