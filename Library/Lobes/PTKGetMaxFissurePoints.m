function [high_fissure_indices, ref_image] = PTKGetMaxFissurePoints(Vx,Vy,Vz,fissure_approximation, lung_mask, PTKfissureness, MYfissureness, image_roi, image_size)
    ...
    [high_fissure_indices, ref_image] = GetFissurePoints(Vx,Vy,Vz,data_points_indices, image_size, lung_mask, PTKfissureness, MYfissureness, image_roi, bin_size);
end


function [maximum_fissureness_indices, ref_image] = GetFissurePoints(Vx,Vy,Vz,indices_for_model, image_size, lung_segmentation, PTKfissureness, MYfissureness, image_roi, bin_size)

    % Get the coordinates of every voxel in the lung segmentation
%     candidate_indices = find(lung_segmentation.RawImage);
    candidate_indices = find(PTKfissureness.RawImage);
...
%     ref_image(candidate_indices) = 2;
    ...
    max_fissureness = max(PTKfissureness.RawImage(candidate_indices));
    ...
    candidate_indices = candidate_indices((PTKfissureness.RawImage(candidate_indices) > fissureness_threshold) & (image_roi.RawImage(candidate_indices) > intensity_threshold));
...
%     candidate_indices = RemoveSmallConnectedStructure(Vx,Vy,Vz,points_coords, lung_segmentation, image_size);
...
function reduced_indices = RemoveSmallConnectedStructure(Vx,Vy,Vz,points_coords, template_image, image_size)
    voxel_volume = prod(template_image.VoxelSize);
...
%     connected_image = tci.RawImage;
    ...
    indices = find(image_mask(:));
    reduced_indices = EigenvectorConnectedComponentAnalysis(Vx,Vy,Vz,indices,image_size,voxel_volume);
%     image_mask = false(image_size);
...
function indices_out = EigenvectorConnectedComponentAnalysis(Vx,Vy,Vz,indices, image_size,voxel_volume)
