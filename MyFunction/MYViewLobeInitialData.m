...
LobeDataMatrixLO=zeros(row_num,col_num,sli_num);LobeDataMatrixROH=zeros(row_num,col_num,sli_num);
new_folder='/hpc/yzha947/CMISSmesh/GenerateDataPoint/LobeTXTData';
mkdir(new_folder);
for i=1:5
    LobeName=LobeNameGroup{i};
    LobeDataFileName=strcat('h11303FRC',LobeName,'LobeData');
    FilePath='/hpc/yzha947/CMISSmesh/GenerateDataPoint/LobeOutput/';
    OriginalFullLobeDataFileName=strcat(FilePath,LobeDataFileName,'.ipdata');
    copyfile(OriginalFullLobeDataFileName,new_folder);
    MovedIpdataFullLobeDataFileName=strcat(new_folder,'/',LobeDataFileName,'.ipdata');
    TXTFullLobeDataFileName=strcat(new_folder,'/',LobeDataFileName,'.txt');
    movefile(MovedIpdataFullLobeDataFileName,TXTFullLobeDataFileName);
    [a1,a2,a3,a4,a5,a6,a7]=textread(TXTFullLobeDataFileName,'%s%s%s%s%s%s%s');
