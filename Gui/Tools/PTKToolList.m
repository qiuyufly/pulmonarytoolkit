classdef PTKToolList < handle
    ...
    properties (Access = private)
        ...
        MarkerPointManager
        CorrectPointManager
        LOCorrectPointManager
        ROCorrectPointManager
        RHCorrectPointManager
    end
    ...
    methods
        function obj = PTKToolList(tool_callback, viewer_panel)
            ...
            obj.CorrectPointManager = MYCorrectPointManager(obj.ViewerPanel, tool_callback);
            obj.LOCorrectPointManager = MYLOCorrectPointManager(obj.ViewerPanel, tool_callback);
            obj.ROCorrectPointManager = MYROCorrectPointManager(obj.ViewerPanel, tool_callback);
            obj.RHCorrectPointManager = MYRHCorrectPointManager(obj.ViewerPanel, tool_callback);
           
            tool_list = {obj.ZoomMatlabTool, obj.PanMatlabTool, obj.MarkerPointManager, obj.WindowLevelTool, obj.CineTool, obj.EditTool, obj.ReplaceColourTool,...
                obj.MapColourTool , obj.CorrectPointManager , obj.LOCorrectPointManager , obj.ROCorrectPointManager , obj.RHCorrectPointManager};
        ...
        function GetMultiCorrectTool(obj)
            LOtool = obj.Tools('MYLOMark');
            ROtool = obj.Tools('MYROMark');
            RHtool = obj.Tools('MYRHMark');
            [LOslice_markers] = LOtool.GetMYCorrectPoints;
            [ROslice_markers] = ROtool.GetMYCorrectPoints;
            [RHslice_markers] = RHtool.GetMYCorrectPoints;
            if ~isempty(LOslice_markers)
                LOtool.Correct(true,LOslice_markers);
            end
            if ~isempty(ROslice_markers)
                ROtool.Correct(true,ROslice_markers);
            end
            if ~isempty(RHslice_markers)
                RHtool.Correct(true,RHslice_markers);
            end
        end
        ...
        function DeleteEditLobe(obj)
            ...
            ROtool = obj.Tools('MYROMark');
            RHtool = obj.Tools('MYRHMark');
            LOtool.DeleteEditLobe;
            ROtool.DeleteEditLobe;
            RHtool.DeleteEditLobe;
        end
