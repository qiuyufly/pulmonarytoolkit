classdef PTKToolbarPanel < PTKPanel
    ...
    methods (Access = private)
        function AddTools(obj)
            ...
            tools_by_category = obj.OrganisedPlugins.GetAllPluginsForMode(obj.ModeName);
            for tool = tools
                obj.AddTool(tool{1}.PluginObject, tools_by_category);
            end
        end
        
        function AddTool(obj, tool, tools_by_category)
            tool_name = class(tool);
            category_key = tool.Category;
            if ~obj.ControlGroups.isKey(category_key)
                current_category_map = tools_by_category(char(category_key));
                new_group = PTKLabelButtonGroup(obj, category_key, '', category_key, current_category_map, obj.ModeName, obj.Reporting);
