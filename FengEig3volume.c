...
void eigen_decomposition(double A[n][n], double V[n][n], double d[n]) {
	...
	/*
	第0个特征值最大的情况（如果第0个特征值最大，就把他和第2个交换，对应的特征向量也交换位置）
	...
	第1个特征值最大的情况（如果第1个特征值最大，就把他和第2个交换，对应的特征向量也交换位置）
	...
	注意和上面不是在同一个if else语句块里面。
	经过上面的交换后，如果第0个特征值大于第1个特征值，就交换特征值和特征向量的位置。）
    if(da[0]>da[1])
    {
