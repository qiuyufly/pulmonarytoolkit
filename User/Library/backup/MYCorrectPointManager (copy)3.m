            ...
            if obj.Enabled
                if obj.ViewerPanel.OverlayImage.ImageExists
                    ...
                    fissure_plane = obj.ViewerPanel.OverlayImage;
                        results_left = MYGetResultsForLeftLobe(lung_mask.Copy, fissure_plane.Copy, reporting);
                    end
                    
%                     lung_mask(preview_fissure==4)=0;
%                     connect_components = bwconncomp(lung_mask, 18);
%                     num_voxels_per_component = cellfun(@numel, connect_components.PixelIdxList);
...
%                     largest_region_indices = connect_components.PixelIdxList{largest_region_index};
%                     second_largest_region_indices = connect_components.PixelIdxList{second_largest_region_index};
%                     lobe_mask = zeros(size(lung_mask));
%                     lobe_mask(largest_region_indices) = 2;
%                     lobe_mask(second_largest_region_indices) = 1;
