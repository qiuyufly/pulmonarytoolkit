classdef MYLOCorrectPointManager < PTKTool
    ...
    methods
        function obj = MYLOCorrectPointManager(viewer_panel, callback)
            obj.ViewerPanel = viewer_panel;
            obj.Callback = callback;
            obj.UndoStack = PTKUndoStack([], 5);
            obj.MarkerPointImage = MYLOCorrectPointImage;
        end
        ...
        function Correct(obj,enable,slice_markers)
            if enable
                ...
                obj.LungMask = lung_mask;
%                     lung_mask_image = lung_mask.RawImage;
%                     lung_mask_image(lung_mask_image~=2) = 0;
%                     lung_mask.ChangeRawImage(lung_mask_image);
%                     
%                     fissure_plane = obj.ViewerPanel.OverlayImage;
%                     results = MYGetResultsForLeftLobe(lung_mask.Copy, fissure_plane.Copy, reporting);
%                     obj.LO_lobe = results;
%                 end
%                 
%                 obj.ViewerPanel.OverlayImage = obj.LO_lobe;
                if ~isempty(obj.CorrectPoints)
                    for i=1:length(slice_markers)
                        original_orientation = obj.ViewerPanel.Orientation;
                        obj.ViewerPanel.Orientation = obj.CorrectPoints(i).orientation;
                        coords=obj.CorrectPoints(i).coords;
                        original_SliceNumber = obj.ViewerPanel.SliceNumber;
                        switch obj.ViewerPanel.Orientation
                            case PTKImageOrientation.Coronal
                                obj.ViewerPanel.SliceNumber(1) = coords(1);
                            case PTKImageOrientation.Sagittal
                                obj.ViewerPanel.SliceNumber(2) = coords(2);
                            case PTKImageOrientation.Axial
                                obj.ViewerPanel.SliceNumber(3) = coords(3);
                        end
                        obj.StartBrush(coords);
                        obj.ApplyBrush(coords);
                        obj.ViewerPanel.Orientation = original_orientation;
                        obj.ViewerPanel.SliceNumber = original_SliceNumber;
                    end
                end
                
                obj.LO_lobe = obj.ViewerPanel.OverlayImage.Copy;
...
%                 SE=[0 1 0;1 1 1;0 1 0];
                ...
                lobe1_overlay_image = imdilate(lobe1_overlay_image,SE);
                lobe2_overlay_image = imdilate(lobe2_overlay_image,SE);
                dt_lobe_overlay_image = lobe1_overlay_image+lobe2_overlay_image;
                
                SE1 = ones(2,2,2);
                fissure_image = zeros(size(new_lobe_overlay_image));
                fissure_image(dt_lobe_overlay_image==2) = 1;
                fissure_image = imerode(fissure_image, SE1);
                
                preview_fissure_index=find(preview_fissure == obj.FissureColour);
                dt_fissure_index=find(fissure_image == 1);
...
%             [closest_colour, second_closest_colour] = obj.GetClosestIndices2D(local_image_coords);
            [closest_colour, second_closest_colour] = obj.GetClosestIndices2D(coords);
...
%                 gaussian_size = max(obj.BrushSize, distance_2d);
                ...
                dt_subimage_second.ChangeRawImage(cropped_image.RawImage == closest_colour);
...
%                 subimage(old_subimage == closest_colour & dt_subimage_second <= 0) = second_closest_colour;
                SE2 = ones(2,2,2);
                subimage = imdilate(subimage, SE2);
                subimage(dt_subimage_second <= 0 & subimage~=closest_colour) = 1;
                subimage(subimage==closest_colour)=0;
                cc = bwconncomp(subimage, 6);
                ...
                correct_maker_index = sub2ind(size(subimage),midpoint(1),midpoint(2),midpoint(3));
                if any(largest_region_indices==correct_maker_index)
                end
        ...
        function [distance, border_distance, border_point] = GetClosestIndices2DForColours(obj, local_image_coords, closest_colour, second_closest_colour)
            ...
            slice_number = obj.ViewerPanel.SliceNumber(orientation);
            image_slice = obj.ViewerPanel.OverlayImage.GetSlice(slice_number, obj.ViewerPanel.Orientation);
            lung_mask_slice = obj.LungMask.GetSlice(slice_number, obj.ViewerPanel.Orientation);
            
%             dt_second_closest =  bwdist(image_slice == second_closest_colour);
            dt_second_closest =  bwdist(image_slice == closest_colour);
        ...
        function AddPointToMarkerImage(obj, marker_position, colour)
            ...
            if obj.MarkerPointImage.ChangeMarkerPoint(coords, colour)
                new_correct_point = [];
                new_correct_point.coords = coords;
                new_correct_point.orientation = obj.ViewerPanel.Orientation;
                if isempty (obj.CorrectPoints)
                    obj.CorrectPoints = new_correct_point;
                else
                    obj.CorrectPoints(end+1) = new_correct_point;
                end
        ...
        function [slice_markers] = GetMYCorrectPoints(obj)
            slice_markers = obj.CorrectPoints;
        end
