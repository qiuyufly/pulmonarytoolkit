...
%                 lobe2_overlay_image = zeros(size(new_lobe_overlay_image));
                preview_fissure1 = zeros(size(new_lobe_overlay_image));
                lobe_mask = obj.LO_lobe.RawImage;
                
                lobe1_overlay_image(new_lobe_overlay_image == obj.ClosestColour)=1;
                distance_lobe_image = bwdist(lobe1_overlay_image);
                preview_fissure1(distance_lobe_image==1 & lobe_mask~=0) = obj.FissureColour;
...
%                 
%                 fissure_matrix = zeros(size(preview_fissure));
%                 SE1 = ones(2,2,2);
%                 fissure_matrix(dt_lobe_overlay_image == 2) = 1;
%                 fissure_matrix = imerode(fissure_matrix,SE1).*double(obj.FissureColour);
                preview_fissure_index = find(preview_fissure == obj.FissureColour);
                dt_fissure_index = find(preview_fissure1 == obj.FissureColour);
...
%                 obj.ApplyEditToImage(lobe2_overlay_image);
                obj.Enabled = enable;
                obj.RemoveAllPoints;
