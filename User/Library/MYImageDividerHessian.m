function [PTKfiltered_image, MYfiltered_image, Vx,Vy,Vz] = MYImageDividerHessian1(image_data, filter_function, mask, gaussian_sigma, hessian_filter_gaussian, dont_divide, dont_calculate_evals, is_left_lung, reporting)
    ...
    %     Example
    %     -------
    %         See the plugins PTKVesselness and PTKFissurenessHessianFactor for
    %         examples.
    %
    %         The following code filters the PTKImage image_data using a function
    ...
    if isempty(is_left_lung)
        reporting.ShowWarning('PTKImageDividerHessian:ArgumentNotSpecified', 'The argument is_left_lung was not specified, so I''m assuming this is true', []);
        is_left_lung = true;
    end
    ...
    Vx = zeros(output_size, 'single');
    Vy = zeros(output_size, 'single');
    Vz = zeros(output_size, 'single');
    ...
    if dont_divide
        ...
        reporting.ShowWarning('PTKImageDividerHessian:NoDivide', 'Ignoring image division and operating on entire lung', []);
        hessian_components = PTKGetHessianComponents(image_data, mask);
        ...
        PTKfiltered_image.ChangeRawImage(filtered_image_raw.PTKfissureness_wrapper.RawImage);
        MYfiltered_image.ChangeRawImage(filtered_image_raw.MYfissureness_wrapper.RawImage);
        ...
        for octant_index = 1 : 8
            ...
            hessian_components = PTKGetHessianComponents(part_image, part_mask);
            ...
            if dont_calculate_evals
                ...
                Dxx = reshape(hessian_components.RawImage(1,:), part_image.ImageSize);
                Dxy = reshape(hessian_components.RawImage(2,:), part_image.ImageSize);
                Dxz = reshape(hessian_components.RawImage(3,:), part_image.ImageSize);
                Dyy = reshape(hessian_components.RawImage(4,:), part_image.ImageSize);
                Dyz = reshape(hessian_components.RawImage(5,:), part_image.ImageSize);
                Dzz = reshape(hessian_components.RawImage(6,:), part_image.ImageSize);
                
               [Lam3,Lam2,Lam1,part_Vx,part_Vy,part_Vz]=MYEig3volume(Dxx,Dxy,Dxz,Dyy,Dyz,Dzz);
