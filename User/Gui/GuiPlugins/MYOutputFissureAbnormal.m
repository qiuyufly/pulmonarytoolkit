classdef MYOutputFissureAbnormal < PTKGuiPlugin
    ...
    properties
        ButtonText = 'Export Abnormal Edit Fissure'
    ...
    methods (Static)
        function RunGuiPlugin(ptk_gui_app)
            ...
            LO_coords_y = original_image_size(2).*image_voxel(2)-LO_coords_y;
            LO_coords_z = -(LO_PTK_fissure_coords_z+image_origin(3)-1).*image_voxel(3);
            LO_coords_z = -(original_image_size(3).*image_voxel(3) + LO_coords_z);
            ...
            RO_coords_y = original_image_size(2).*image_voxel(2)-RO_coords_y;
            RO_coords_z = -(RO_PTK_fissure_coords_z+image_origin(3)-1).*image_voxel(3);
            RO_coords_z = -(original_image_size(3).*image_voxel(3) + RO_coords_z);
            ...
            RH_coords_y = original_image_size(2).*image_voxel(2)-RH_coords_y;
            RH_coords_z = -(RH_PTK_fissure_coords_z+image_origin(3)-1).*image_voxel(3);
            RH_coords_z = -(original_image_size(3).*image_voxel(3) + RH_coords_z);
            ...
            MYWriteExdata('fissure_LObliquetrimmed.exdata',LO_coords,'fissure_LOblique',400,save_full_path);
