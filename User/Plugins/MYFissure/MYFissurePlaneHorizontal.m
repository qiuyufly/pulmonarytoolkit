classdef MYFissurePlaneHorizontal < PTKPlugin
    ...
    properties
        ButtonText = 'My Fissure Plane<br>horizontal'
        ToolTip = ''
        Category = 'Fissures'
    ...
    methods (Static)
        function results = RunPlugin(application, reporting)
            ...
            max_fissure_points = application.GetResult('MYMaximumFissurePointsHorizontal');            
            right_upper_lung = application.GetResult('MYLobesFromFissurePlaneOblique');
            right_upper_lung.ChangeRawImage(right_upper_lung.RawImage == 1);
            
            results = MYFissurePlaneHorizontal.GetRightLungResults(max_fissure_points, right_upper_lung, application.GetResult('PTKGetRightLungROI'), reporting);
            if ~isempty(results)
